...
const SelectJob = ({navigation}) => {
    ...
    return (
         <View style={{ padding: 40 }}>
            <CreateTaskHeader header1={"Yeni Görev Ekle"} header2={"Görev Ata"} isrequired={true} />
            <FlatList
                data={data}
                keyExtractor={item => item.index}
                renderItem={(item) => <SelectJobItem data={item} key={item.index.toString()}/>}
                style={{height:Dimensions.get("screen").height/1.5,marginVertical:20}}
            />
            <CreateTaskFooter navigation={navigation} navigationPage={"SelectJob"} nextPage={"JobInfo"} />
        </View>
    )
}
